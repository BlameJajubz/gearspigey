...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'gearspigey' | 'penis1':
				curStage = 'gearspigey';
				defaultCamZoom = 0.6;

				var sky:BGSprite = new BGSprite('sky', -1300, -900, 0.7, 0.7); 
				add (sky);

				var midground:BGSprite = new BGSprite('midground', -600, -200, 0.9, 0.9);
				add(midground);

				var ground:BGSprite = new BGSprite('ground', -900, 250, 1, 1); 
				ground.setGraphicSize(Std.int(ground.width * 6)); //change this to literally anything but 6 later lol
				ground.updateHitbox();
				add(ground);
